...
class DijitalKumbara(QWidget):
    def __init__(self):
        ...
        # Toplam parayı tutacak olan backend değişkenimiz
        self.toplam_bakiye = 0

        # Pencere Ayarları
        self.setWindowTitle("Dijital Kumbara")
        self.resize(380, 250)
        self.setStyleSheet("background-color: #1e1e2e; color: #cdd6f4;") # Koyu tema siber renkler :)

        # --- ARAYÜZ ELEMANLARINI OLUŞTURMA ---

        # 1. Toplam Parayı Gösterecek Büyük Yazı Alanı (QLabel)
        ...
        # 2. Kullanıcının Para Miktarını Yazacağı Kutu (QLineEdit)
        ...
        # Mühendislik Dokunuşu: Kutunun içine harf yazılmasını engelliyoruz, sadece tam sayı girilebilsin
        self.input_miktar.setValidator(QIntValidator(1, 999999, self))

        # 3. Para Atma Butonu (QPushButton)
        ...
        # 4. Para Alma Butonu (QPushButton) - YENİ EKLENDİ
        ...
        # --- BAĞLANTI (SIGNAL / SLOT) ---
        self.btn_para_at.clicked.connect(self.para_ekle)
        self.btn_para_al.clicked.connect(self.para_al) # Yeni bağlantı
        self.input_miktar.returnPressed.connect(self.para_ekle) # Enter'a basınca varsayılan olarak para eklesin

        # --- YERLEŞİM (LAYOUT) ---

        # Butonları yan yana koymak için yatay bir kutu oluşturuyoruz
    ...
    def para_ekle(self):
        ...
        if girilen_metin:
            ...
            self.input_miktar.setPlaceholderText("Miktar girin...") # Hata mesajı varsa sıfırla

    # YENİ FONKSİYON: Para Alma Mantığı
    def para_al(self):
        girilen_metin = self.input_miktar.text()
        ...
        if girilen_metin:
            ...
            # Kumbarada yeterli para var mı kontrolü
            if alinacak_para <= self.toplam_bakiye:
                ...
                # Bakiye yetersizse kullanıcıyı uyar ve kutuyu temizle
                self.input_miktar.clear()
                self.input_miktar.setPlaceholderText("Bakiye yetersiz!")

# --- PROGRAMI BAŞLATAN ANA MOTOR ---
